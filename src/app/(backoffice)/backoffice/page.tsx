...
export default function BackofficeLogin() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (authError) { setError('Falha na autenticacao. Verifique suas credenciais.'); return }
            if (data.session) { router.push('/backoffice/dashboard'); router.refresh() }
        ...
        <div className="min-h-screen flex items-center justify-center px-6">
            <div className="w-full max-w-sm">
                <div className="text-center mb-10">
                    ...
                    <p className="text-xs text-imi-400 uppercase tracking-[0.2em] font-bold">Plataforma de Inteligencia Imobiliaria</p>
                </div>
                <form onSubmit={handleLogin} className="space-y-5">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-imi-300 hover:text-imi-600 transition-colors">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                        </div>
                    </div>
                ...
                <p className="mt-10 text-center text-[10px] text-imi-300 uppercase tracking-widest">2026 IMI</p>
            </div>
        </div>
    )
}
